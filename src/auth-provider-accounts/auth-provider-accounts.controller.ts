...
export class AuthProviderAccountsController {
  constructor(
    private readonly authProviderAccountsService: AuthProviderAccountsService
  ) {}

  @Post()
  create(@Body() createAuthProviderAccountDto: CreateAuthProviderAccountDto) {
    return this.authProviderAccountsService.create(
      createAuthProviderAccountDto
    );
  }
  ...
  update(
    @Param('id') id: string,
    @Body() updateAuthProviderAccountDto: UpdateAuthProviderAccountDto
  ) {
    return this.authProviderAccountsService.update(
      +id,
      updateAuthProviderAccountDto
    );
  }
